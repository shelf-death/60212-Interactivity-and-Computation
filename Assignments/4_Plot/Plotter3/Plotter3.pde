...
void draw(){
  background(255);
  if (savePDF == true){ beginRaw(PDF, "export.pdf");}
  
  beginShape();
  //Iterate over path array drawing lines
  for (int p = 0; p < paths.size(); p++){
    if (paths.get(p).end.z > 0 && p < paths.size()-1) {
      for (float step = 0; step < numSteps; step += 2){
        PVector interpolated = paths.get(p).end;
        interpolated.lerp(paths.get(p+1).end.x,paths.get(p+1).end.y,paths.get(p+1).end.z,step/numSteps);
        stroke(30);
        curveVertex(interpolated.x,interpolated.y,interpolated.z);
      }//
    }//end z depth check
    else{
    stroke(0);
